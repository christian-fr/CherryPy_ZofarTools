...
class QmlReader:
    ...
    def __init__(self, filename, file=None, create_graph=False, draw=False, truncate=False):
        self.filename = filename
        self.tmp = []
        self.logger = logging.getLogger('debug')
        self.png_full_path = None
        ...
        self.questionnaire = Questionnaire.Questionnaire(filename)
        ...
        self.init_pgv_graph()
        #nx.write_gml(self.DiGraph, path.splitext(filename)[0] + '.gml')
        self.prepare_pgv_graph()
        #self.draw_pgv_graph(output_file=path.splitext(filename)[0] + '.png')
    ...
    def prepare_pgv_graph(self):
        output_folder = str(path.join(str(path.split(self.filename)[0]), 'flowcharts'))
        ...
        filename = timestamp + '_' + path.splitext(path.split(self.filename)[1])[0]
        ...
        self.png_full_path = path.join(output_folder, filename + '.png')
        self.png_filename = path.join(filename + '.png')
